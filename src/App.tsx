...
type Tab = "overview" | "dashboard" | "buttons" | "performance" | "profiles" | "tools" | "tester" | "diagnostics" | "backup" | "autoSwitch" | "help" | "support" | "assistant" | "settings";
...
const tabs: { id: Tab; label: string; icon: typeof Mouse }[] = [
  ...
  { id: "buttons", label: "Buttons", icon: Mouse }, { id: "performance", label: "Performance", icon: Gauge }, { id: "profiles", label: "Profiles", icon: Gamepad2 },
  { id: "tester", label: "Mouse tester", icon: Mouse }, { id: "diagnostics", label: "Device diagnostics", icon: RefreshCw }, { id: "backup", label: "Profile backup", icon: BookOpen }, { id: "autoSwitch", label: "Auto-switching", icon: Play },
  { id: "assistant", label: "Serx", icon: Bot }, { id: "help", label: "Guides", icon: BookOpen }, { id: "support", label: "Quick fixes", icon: Settings }, { id: "settings", label: "Settings", icon: Settings },
];
const sidebarGroups: { label: string; ids: Tab[] }[] = [
  { label: "Device", ids: ["overview", "dashboard"] },
  { label: "Configure", ids: ["buttons", "performance", "profiles"] },
  { label: "Tools", ids: ["tester", "diagnostics", "backup", "autoSwitch"] },
  { label: "Assistant", ids: ["assistant"] },
  { label: "Support", ids: ["help", "support"] },
];
...
export default function App() {
  ...
  const [testerClosing, setTesterClosing] = useState(false);
  ...
  const toggleMouseTester = () => {
    if (!testerOpen) {
      setTesterClosing(false);
      setTesterOpen(true);
      return;
    }
    if (testerClosing) return;
    setTesterClosing(true);
    window.setTimeout(() => {
      setTesterOpen(false);
      setTesterClosing(false);
    }, 360);
  ...
  const importProfiles = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const restored = backup.profiles.filter(item => item && typeof item.name === "string").map(item => ({ ...makeProfile(item.name), ...item, id: crypto.randomUUID(), buttons: cloneButtons(item.buttons) }));
  ...
  return <div className={`app-shell glass-${glassMode}`} style={glassStyle}>
    ...
    <div className="ui-scale-layer" style={{ "--ui-scale": uiScale / 100 } as React.CSSProperties}>
      <aside className="sidebar glass-surface">
        ...
        <section className="current-area" aria-label="Current device setup"><div className="sidebar-section-label">Current</div><section className="current-setup glass-control"><span>Current setup</span><strong>{activeProfile?.name || "Default"}</strong><div><b>{dpi} DPI</b><b>{polling}</b></div><small>{connectionLabel}</small><button onClick={() => changeTab("dashboard")} type="button">Open dashboard →</button><img className="current-device-image" key={deviceImage} src={deviceImage} alt="" aria-hidden="true"/><div className="sidebar-device-card glass-surface"><span className={`device-dot ${connected ? "online" : ""}`}/><div className="device-card-copy"><strong>{mouse?.name || "No mouse detected"}</strong><span>{connectionLabel}</span>{isX1 && batteryLevel !== null && <span className="battery-readout battery-bars" role="img" aria-label="Mouse battery status"><i className={batteryLevel > 0 ? "filled" : ""}/><i className={batteryLevel > 34 ? "filled" : ""}/><i className={batteryLevel > 67 ? "filled" : ""}/></span>}</div><button className="icon-button" onClick={() => void refresh()} type="button"><RefreshCw size={15} className={loading ? "spin" : ""}/></button></div></section></section><button className={`nav-item settings-nav ${tab === "settings" ? "active" : ""}`} onClick={() => changeTab("settings")} type="button"><Settings size={18}/><span>Settings</span></button>
        <div className="sidebar-footer">Unnamed Desktop App <span>•</span> v0.1.0</div>
      </aside>
      <main className="content">
        <header className="topbar"><div>{tab === "overview" ? <><div className="eyebrow">Your setup</div><h1>Good evening{userName.trim() ? `, ${userName.trim()}` : ""}</h1><p className="welcome-subtitle">{connected ? `${mouse?.name || "Your mouse"} is ready` : "Let’s get your mouse set up"}</p></> : <><div className="eyebrow">{tabs.find(t => t.id === tab)?.label}</div><h1>{tabs.find(t => t.id === tab)?.label}</h1></>}</div><div className="topbar-actions"><span className="profile-label">Active profile</span><select className="profile-select glass-control" value={activeProfile?.id || ""} onChange={e => setProfile(e.target.value)}>{profiles.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div></header>
        {error && <div className="error-banner">{error}</div>}
        {tab === "overview" && <><section className="device-hero panel glass-surface"><div className="device-hero-copy"><div className="status-pill glass-control"><span className={`device-dot ${connected ? "online" : ""}`}/>{connected ? "Connected" : "Not connected"}</div><h2>{mouse?.name || "No mouse detected"}</h2><p>{connected ? "Connected and ready" : "Connect a mouse to begin configuring it"}</p><div className="device-meta">{mouse?.connection && <span>{mouse.connection}</span>}{mouse?.manufacturer && <span>{mouse.manufacturer}</span>}{mouse?.vid && mouse?.pid && <span>VID {mouse.vid} · PID {mouse.pid}</span>}</div><button className="primary-button glass-control" onClick={() => void refresh()} disabled={loading} type="button"><RefreshCw size={16}/> {loading ? "Scanning…" : "Scan for devices"}</button></div><div className="mouse-illustration"><img src={deviceImage} alt={isG305 ? "Black Logitech G305, top view" : isModelO ? "Black Glorious Model O Wired, top view" : "White Attack Shark X1, top view"}/></div></section><div className="section-heading"><div><h2>Quick settings</h2><p>Common controls for your current profile.</p></div></div><section className="settings-grid overview-settings"><div className="setting-card panel glass-surface"><div className="setting-icon"><Gauge size={19}/></div><div className="setting-copy"><DpiInput value={dpi} onApply={applyDpi} compact minimum={dpiMinimum} maximum={dpiMaximum}/><p className="setting-description">DPI means dots per inch. A higher value makes the pointer travel farther for the same hand movement; lower feels slower and more precise.</p></div><input type="range" min={dpiMinimum} max={dpiMaximum} step="50" value={dpi} onChange={e => applyDpi(Number(e.target.value))}/><div className="dpi-preset-row">{dpiPresets.slice(0, 4).map(value => <button className={dpi === value ? "selected" : ""} key={value} onClick={() => applyDpi(value)} type="button">{value >= 1000 ? `${value / 1000}K` : value}</button>)}</div><small className="muted-warning">{isG305 ? "G305 range · 200–12,000 DPI." : isModelO ? "Model O range · 50–12,000 DPI." : "Hardware DPI control · 50–40,000 DPI."}</small></div><div className="setting-card panel glass-surface"><div className="setting-icon"><Zap size={19}/></div><div className="setting-copy"><strong>{polling}</strong><p className="setting-description">Polling rate is how often the mouse reports movement to Windows. Higher rates can feel more responsive, while lower rates can save a little power.</p></div><div className="segmented">{["125 Hz","500 Hz","1000 Hz"].map(r => <button className={polling === r ? "selected" : ""} key={r} onClick={() => setPollingForProfile(r)} type="button">{r.replace(" Hz","")}</button>)}</div></div><div className="setting-card panel glass-surface"><div className="setting-icon"><Keyboard size={19}/></div><div className="setting-copy"><strong>{deviceButtons.length} buttons</strong><p className="setting-description">Choose what each physical mouse button does. You can keep the defaults, send a keybind, or use handy Windows shortcuts.</p></div><button className="secondary-button glass-control" onClick={() => changeTab("buttons")} type="button">Configure</button></div></section></>}
        {tab === "dashboard" && <section className="panel page-panel glass-surface dashboard-panel"><div className="section-heading compact"><div><h2>Device dashboard</h2><p>Live details reported by the mouse currently connected to Unnamed.</p></div><button className="secondary-button" onClick={() => void refresh()} type="button"><RefreshCw size={15}/> Refresh</button></div><div className="dashboard-grid"><article className="dashboard-device glass-control"><div><span className={connected ? "device-dot online" : "device-dot"}/><b>{connected ? "Connected" : "Not connected"}</b></div><h3>{mouse?.name || "No mouse detected"}</h3><p>{connected ? connectionLabel : "Connect a mouse and scan for details."}</p><div className="dashboard-meta"><span>VID <b>{mouse?.vid || "—"}</b></span><span>PID <b>{mouse?.pid || "—"}</b></span></div><img src={deviceImage} alt="Connected mouse"/></article><article className="dashboard-capabilities glass-control"><h3>Current configuration</h3><div className="capability-grid"><div><span>DPI</span><strong>{dpi}</strong><small>{dpiMinimum}–{dpiMaximum.toLocaleString()} range</small></div><div><span>Report rate</span><strong>{polling}</strong><small>Current profile</small></div><div><span>Buttons</span><strong>{deviceButtons.length}</strong><small>Configurable inputs</small></div><div><span>Connection</span><strong>{connectionLabel}</strong><small>Detected connection</small></div></div></article></div><article className="dashboard-note glass-control"><strong>Hardware details</strong><p>Sensor name, battery level, firmware version, and RGB support only appear when the mouse reports them. Unnamed does not guess or send unverified hardware commands.</p></article></section>}
        {tab === "tester" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>Mouse tester</h2><p>Test clicks, side buttons, and scroll without changing any mappings.</p></div></div><section className="mouse-tester glass-control"><header><div><span>Live mouse tester</span><h3>{testerStatus}</h3></div><button className="secondary-button" onClick={() => { setTestedInputs([]); setTesterStatus("Waiting for an input"); }} type="button">Clear</button></header><div className="mouse-tester-pad" tabIndex={0} onMouseDown={recordMouseInput} onWheel={recordScrollInput} onContextMenu={event => event.preventDefault()}><strong>Click or scroll here</strong><span>Use left, right, wheel, or either side button while this area is focused.</span><div className="mouse-tester-inputs">{["Button 1","Button 2","Button 3","Button 4","Button 5","Wheel up","Wheel down"].map(input => <div className={testedInputs.includes(input) ? "detected" : ""} key={input}><b>{input.replace("Button ", "M").replace("Wheel ", "Scroll ")}</b><span>{testedInputs.includes(input) ? "Detected" : "Waiting"}</span></div>)}</div></div><p>It only observes inputs in this tester area—it does not change your button mappings.</p></section></section>}
        {tab === "diagnostics" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>Device diagnostics</h2><p>Read the connected HID interfaces without changing mouse settings.</p></div><button className="secondary-button" onClick={() => void inspectDpiHardware()} type="button">Run diagnostics</button></div>{dpiDiagnostics ? <pre className="diagnostics-output glass-control">{dpiDiagnostics}</pre> : <div className="empty-state glass-control">Run diagnostics to inspect reported HID interfaces and feature reports.</div>}</section>}
        {tab === "backup" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>Profile backup</h2><p>Export your profiles to keep a backup or move them to another PC.</p></div></div><article className="tool-card glass-control"><div className="tool-actions"><button className="secondary-button" onClick={exportProfiles} type="button">Export profiles</button><button className="secondary-button" onClick={() => profileImportRef.current?.click()} type="button">Import profiles</button><input ref={profileImportRef} className="file-input" type="file" accept="application/json" onChange={importProfiles}/></div><small>Imports replace the current local profile list.</small></article></section>}
        {tab === "autoSwitch" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>Auto-switching</h2><p>Apply a profile automatically when a selected Windows app is open.</p></div></div><article className="tool-card glass-control"><label className="toggle-row"><input type="checkbox" checked={autoSwitchEnabled} onChange={e => setAutoSwitchEnabled(e.target.checked)}/><span>Enable auto-switching</span></label><label>Program name<input value={watchedProgram} placeholder="e.g. Valorant-Win64-Shipping.exe" onChange={e => setWatchedProgram(e.target.value)}/></label><label>Use profile<select value={autoSwitchProfile} onChange={e => setAutoSwitchProfile(e.target.value)}><option value="">Choose a profile</option>{profiles.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><small>{autoSwitchStatus}</small></article></section>}
        {tab === "support" && <section className="panel page-panel glass-surface help-panel"><div className="section-heading compact"><div><h2>Quick fixes</h2><p>Safe first steps when something does not look right.</p></div></div><div className="help-grid"><article className="help-card glass-control"><h3>Mouse not detected</h3><p>Reconnect the USB cable or receiver, then choose Scan for devices from Overview.</p></article><article className="help-card glass-control"><h3>DPI did not change</h3><p>For the X1, confirm it is connected and try the value again. Other mice need a verified hardware protocol.</p></article><article className="help-card glass-control"><h3>Buttons behave oddly</h3><p>Close other mouse software while testing, as it can override the mapping.</p></article></div></section>}
        {tab === "buttons" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>Button mapping</h2><p>Choose a button, then configure its action in the editor below.</p></div><div className="page-status"><span className="device-dot online"/>Profile saved</div></div><div className="mapping-label-row"><span>Button</span><span>Control</span><span>Assigned action</span><span>Test</span></div><div className="button-map-grid">{deviceButtons.map(button => { const binding = buttons[button] || { action: "Default" as ButtonAction }; return <button className={`map-row glass-control ${selectedButton === button ? "editing" : ""}`} key={button} onClick={() => setSelectedButton(button)} type="button"><span className="button-key">{button.replace("Button ","M")}</span><span>{button}</span><span className="map-action-summary">{binding.action === "Keybind" ? binding.target || "Set keybind" : binding.action}</span><span className="map-test" title="Edit this button">›</span></button>; })}</div><div className="mouse-tester-toggle-row"><button className={`secondary-button glass-control mouse-tester-toggle ${testerOpen ? "selected" : ""}`} onClick={toggleMouseTester} type="button">◉ {testerOpen ? "Close mouse tester" : "Test mouse inputs"}</button><span>Check clicks, side buttons, and scroll in this window.</span></div>{testerOpen && <section className={`mouse-tester glass-control ${testerClosing ? "closing" : ""}`}><header><div><span>Live mouse tester</span><h3>{testerStatus}</h3></div><button className="secondary-button" onClick={() => { setTestedInputs([]); setTesterStatus("Waiting for an input"); }} type="button">Clear</button></header><div className="mouse-tester-pad" tabIndex={0} onMouseDown={recordMouseInput} onWheel={recordScrollInput} onContextMenu={event => event.preventDefault()}><strong>Click or scroll here</strong><span>Use left, right, wheel, or either side button while this area is focused.</span><div className="mouse-tester-inputs">{["Button 1","Button 2","Button 3","Button 4","Button 5","Wheel up","Wheel down"].map(input => <div className={testedInputs.includes(input) ? "detected" : ""} key={input}><b>{input.replace("Button ", "M").replace("Wheel ", "Scroll ")}</b><span>{testedInputs.includes(input) ? "Detected" : "Waiting"}</span></div>)}</div></div><p>It only observes inputs in this tester area—it does not change your button mappings.</p></section>}<div className="button-workspace"><div className="mouse-button-map glass-control"><div className="mouse-map-heading"><div><span>Interactive layout</span><h3>{isG305 ? "Logitech G305 LIGHTSPEED" : isModelO ? "Glorious Model O Wired" : "Attack Shark X1"}</h3></div><span>{selectedButton.replace("Button ", "M")} selected</span></div><div className={`mouse-map-canvas ${isG305 ? "g305-map" : isModelO ? "model-o-map" : "x1-map"}${sideButtonView ? " side-view-map" : ""}`}><img className="button-map-image" key={buttonMapImage} src={buttonMapImage} alt={sideButtonView ? "Attack Shark X1 side button map" : isG305 ? "Logitech G305 button map" : isModelO ? "Glorious Model O Wired button map" : "Attack Shark X1 button map"}/>{mouseHotspots.filter(zone => deviceButtons.includes(zone.button)).map(zone => <button className={`${zone.className} ${selectedButton === zone.button ? "selected" : ""}`} key={zone.button} onClick={() => setSelectedButton(zone.button)} type="button" aria-label={`Edit ${zone.label}`}><b>{zone.button.replace("Button ", "M")}</b><span>{zone.label}</span></button>)}</div><p>Click a highlighted button on the mouse to edit it.</p></div>{(() => { const binding = buttons[selectedButton] || { action: "Default" as ButtonAction }; return <div className="button-editor glass-control"><div className="editor-heading"><div><span>Editing</span><h3>{selectedButton.replace("Button ", "M")} · {selectedButton}</h3></div><button className="map-test" onClick={() => void testButtonAction(binding)} type="button" title="Test this action"><Play size={14}/></button></div><div className="editor-section"><span>Action</span><select value={binding.action} onChange={e => updateButton(selectedButton, { action: e.target.value as ButtonAction, target: ["Keybind", "Custom program"].includes(e.target.value) ? binding.target || "" : undefined })}>{actions.map(action => <option key={action}>{action}</option>)}</select><div className="quick-actions"><button className={binding.action === "Keybind" ? "selected" : ""} onClick={() => updateButton(selectedButton, { action: "Keybind", target: binding.action === "Keybind" ? binding.target : "" })} type="button">⌨ Custom keybind</button><button className={binding.action === "Open Email" ? "selected" : ""} onClick={() => updateButton(selectedButton, { action: "Open Email" })} type="button">✉ Open email</button></div></div>{binding.action === "Keybind" && <div className="editor-section keybind-section"><span>Custom keybind</span><KeybindInput value={binding.target || ""} onChange={target => updateButton(selectedButton, { target })}/><div className="keybind-presets">{keybindPresets.map(key => <button key={key} onClick={() => updateButton(selectedButton, { target: key })} className={binding.target === key ? "selected" : ""} type="button">{key}</button>)}</div></div>}{binding.action === "Custom program" && <div className="editor-section"><span>Program path</span><input className="map-target" value={binding.target || ""} placeholder="e.g. notepad.exe" onChange={e => updateButton(selectedButton, { target: e.target.value })}/></div>}<p>{selectedButton === "Button 4" || selectedButton === "Button 5" ? "This action runs while Unnamed is open." : "This profile setting is saved. Hardware remapping for M1–M3 needs the X1’s native command format."}</p></div>; })()}</div><p className="mapping-note">Presets can launch Explorer, Task Manager, Settings, or your default email app. Keybinds send to whichever app is currently active.</p></section>}
        ...
        {tab === "performance" && <section className="panel page-panel glass-surface folded-utilities"><div className="section-heading compact"><div><h2>Performance tools</h2><p>Useful checks and conversions, kept with your performance settings.</p></div></div><div className="tools-grid compact-tools"><article className="tool-card glass-control"><h3>DPI converter</h3><p>Keep the same effective sensitivity when changing DPI.</p><div className="converter-grid"><label>Current DPI<input type="number" min="50" value={converterDpi} onChange={e => setConverterDpi(Math.max(50, Number(e.target.value) || 50))}/></label><label>Sensitivity<input type="number" min="0.001" step="0.01" value={converterSensitivity} onChange={e => setConverterSensitivity(Math.max(.001, Number(e.target.value) || .001))}/></label><label>New DPI<input type="number" min="50" value={targetDpi} onChange={e => setTargetDpi(Math.max(50, Number(e.target.value) || 50))}/></label></div><div className="converter-result"><span>New sensitivity</span><strong>{(converterDpi * converterSensitivity / targetDpi).toFixed(3)}</strong></div></article><article className="tool-card glass-control"><h3>Sensor & device check</h3><p>Record a few quick physical checks before gaming.</p><div className="surface-checks">{["Smooth tracking","Scroll movement","Lift-off check"].map(check => <button className={surfaceChecks.includes(check) ? "selected" : ""} onClick={() => recordSurfaceCheck(check)} key={check} type="button">{surfaceChecks.includes(check) ? "✓ " : ""}{check}</button>)}</div><small>{surfaceChecks.length ? `${surfaceChecks.length}/3 checks recorded` : "No checks recorded yet."}</small><div className="support-inline"><b>{mouse?.name || "No mouse detected"}</b><span>{mouse ? (isG305 || isModelO ? "Detected · layout supported · hardware control needs verification" : "Detected · X1 DPI hardware control supported") : "Connect a mouse and scan for devices."}</span></div></article></div></section>}
        ...
        {tab === "tools" && <section className="panel page-panel glass-surface tools-panel"><div className="section-heading compact"><div><h2>Tools</h2><p>Useful extras for profiles, sensitivity, and hardware checks.</p></div></div><div className="tools-grid">
          ...
          <article className="tool-card glass-control"><h3>Profile backup</h3><p>Move your saved profiles between PCs or keep a backup.</p><div className="tool-actions"><button className="secondary-button" onClick={exportProfiles} type="button">Export profiles</button><button className="secondary-button" onClick={() => profileImportRef.current?.click()} type="button">Import profiles</button><input ref={profileImportRef} className="file-input" type="file" accept="application/json" onChange={importProfiles}/></div><small>Imports replace the current local profile list.</small></article>
          <article className="tool-card glass-control"><h3>Sensor & device check</h3><p>Quickly record a few physical checks before gaming.</p><div className="surface-checks">{["Smooth tracking","Scroll movement","Lift-off check"].map(check => <button className={surfaceChecks.includes(check) ? "selected" : ""} onClick={() => recordSurfaceCheck(check)} key={check} type="button">{surfaceChecks.includes(check) ? "✓ " : ""}{check}</button>)}</div><small>{surfaceChecks.length ? `${surfaceChecks.length}/3 checks recorded` : "No checks recorded yet."}</small></article>
          <article className="tool-card glass-control support-hub"><h3>Device support</h3><p>Clear status for the device currently selected in Unnamed.</p><div><b>{mouse?.name || "No mouse detected"}</b><span>{mouse ? (isG305 || isModelO ? "Detected · layout supported · hardware control needs verification" : "Detected · X1 DPI hardware control supported") : "Connect a mouse and scan for devices."}</span></div><div><b>RGB</b><span>Awaiting a verified per-device hardware protocol.</span></div><small>We won’t send guessed RGB commands to a mouse.</small></article>
        </div></section>}
        ...
        {tab === "settings" && <section className="panel page-panel background-settings glass-surface"><div className="section-heading compact"><div><h2>Appearance</h2><p>Make the background and interface as transparent as you want.</p></div></div>
          <div className="appearance-section identity-section"><h3>Make it yours</h3><p className="appearance-note">Your name is saved on this PC and appears in the welcome message.</p><label className="name-setting">Your name<input value={userName} maxLength={30} placeholder="e.g. Serx" onChange={event => setUserName(event.target.value)} /></label></div><div className="appearance-section"><h3>Background</h3><div className="background-mode-grid">{([["default","Default"],["solid","Solid colour"],["gradient","Gradient"],["image","Image"]] as [BackgroundMode,string][]).map(([mode,label]) => <button className={`background-mode glass-control ${bgMode === mode ? "selected" : ""}`} key={mode} onClick={() => setBgMode(mode)} type="button"><Palette size={17}/><span>{label}</span></button>)}</div>
