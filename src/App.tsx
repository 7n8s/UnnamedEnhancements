...
const prepareBackground = async (file: File) => {
  // Preserve the original bytes. Upscaling and WebP re-encoding softened small
  // images and could introduce halos around transparent artwork.
...
export default function App() {
  ...
  const [accentColor, setAccentColor] = useState(() => localStorage.getItem("unnamed-accent-color-v2") || "#8DFFC1");
  ...
  const [glassOpacity, setGlassOpacity] = useState(() => Number(localStorage.getItem("unnamed-glass-opacity") || 72));
  const [glassBlur, setGlassBlur] = useState(() => Number(localStorage.getItem("unnamed-glass-blur") || 26));
  const [glassTint, setGlassTint] = useState(() => localStorage.getItem("unnamed-glass-tint-v2") || "#0C1418");
  ...
  const resetAppearance = () => { setBgMode("default"); setBgImage(""); setBgFocus("center"); setBgOpacity(100); setBgBlur(0); setBgSaturation(100); setGlassMode("regular"); setGlassOpacity(72); setGlassBlur(26); setGlassTint("#0C1418"); setGlassBorder(28); setGlassRadius(22); setUiScale(100); setTextScale(100); setAccentColor("#8DFFC1"); setLayoutDensity("comfortable"); setMotionEnabled(true); };

  const background: React.CSSProperties = bgMode === "solid" ? { background: bgColor } : bgMode === "gradient" ? { background: `linear-gradient(135deg, ${gradA}, ${gradB})` } : bgMode === "image" && bgImage ? { backgroundImage: `url(${bgImage})`, backgroundSize: fit === "stretch" ? "100% 100%" : fit, backgroundPosition: bgFocus, backgroundRepeat: "no-repeat" } : { background: "radial-gradient(circle at 16% 8%, rgba(45, 255, 170, .10), transparent 24%), radial-gradient(circle at 86% 18%, rgba(83, 128, 255, .10), transparent 31%), linear-gradient(150deg, #071014 0%, #0a0f13 48%, #06090c 100%)" };
  ...
  return <div className={"app-shell background-"+bgMode+" "+deviceClass+" glass-"+glassMode+" density-"+layoutDensity+" "+(motionEnabled ? "motion-on" : "motion-off")} style={glassStyle}>
    ...
    <div className="ui-scale-layer" style={{ "--ui-scale": uiScale / 100 } as React.CSSProperties}>
      ...
      <main className="content" id="main-content">
        ...
        {tab === "performance" && <>
          <section className="panel sensitivity-panel"><div className="card-heading"><span className="icon-tile"><Gauge size={21}/></span><div><h2>Your sensitivity</h2><p>{canChangeDpi ? `Applied directly to your ${dpiDeviceName}.` : "Connect a supported mouse for hardware adjustment."}</p></div><span className="subtle-badge">{canChangeDpi ? "Hardware control" : "Unavailable"}</span></div>
        ...
        {tab === "profiles" && <>
          <section className="panel"><div className="card-heading"><div><h2>Your saved setups</h2><p>DPI and button assignments, saved on this PC.</p></div><span className="subtle-badge">{profiles.length} {profiles.length === 1 ? "profile" : "profiles"}</span></div><div className="profile-create"><input aria-label="New profile name" maxLength={40} value={newProfileName} placeholder="Name a new profile…" onChange={e => setNewProfileName(e.target.value)} onKeyDown={e => e.key === "Enter" && createProfile()}/><button className="primary-button" onClick={createProfile} disabled={!newProfileName.trim()}><Plus size={16}/>Create profile</button></div>
            <div className="profile-grid">{profiles.map(item => <article className={"profile-card "+(activeProfile?.id === item.id ? "selected" : "")} key={item.id} onClick={() => { if (activeProfile?.id !== item.id) setProfile(item.id); }} role="button" tabIndex={0} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setProfile(item.id); } }}><div className="profile-card-top"><Gamepad2 size={23}/>{activeProfile?.id === item.id && <span className="subtle-badge"><Check size={12}/>Active</span>}</div><h3>{item.name}</h3><p>{item.dpi.toLocaleString()} DPI <span>·</span> {Object.values(item.buttons).filter(b => !["Default","Back","Forward"].includes(b.action)).length} custom actions</p><div className="profile-card-actions"><span className="profile-use-label">{activeProfile?.id === item.id ? "In use" : "Click to apply"}</span><button className="text-button" onClick={event => { event.stopPropagation(); setRenameTarget(item); setRenameText(item.name); }}>Rename</button></div></article>)}</div>
          </section>
          <section className="panel profile-utilities"><details><summary><span><ArrowDownToLine size={19}/>Backup & restore<small>Take your profiles with you.</small></span><ChevronRight size={17}/></summary><div className="details-body"><p>Export a backup before moving PCs. Importing replaces your current profiles.</p><div className="tool-actions"><button className="secondary-button" onClick={exportProfiles}><ArrowDownToLine size={15}/>Export profiles</button><button className="secondary-button" onClick={() => profileImportRef.current?.click()}><Upload size={15}/>Import backup</button><input ref={profileImportRef} className="file-input" type="file" accept="application/json" onChange={importProfiles}/></div></div></details>
          </section>
        </>}
